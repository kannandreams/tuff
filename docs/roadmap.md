...
This roadmap tracks the public direction for Loadout. The sequence is designed
to keep the core lifecycle stable before expanding into executable primitives,
remote distribution, and policy.

## Product direction

Loadout should solve lifecycle management for agent capabilities:
...
These features make Loadout easier to adopt in real teams after the main
...
| [Project diagnostics with `loadout doctor`](https://github.com/kannandreams/loadout/issues/13) | Engineers need one command to explain broken state. |
