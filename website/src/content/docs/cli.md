...
A git install that follows a commit is checked with one `ls-remote` and no clone, unless the source declares a version and the commit moved, in which case the clone is what reads the version it declares now. The same listing says whether the repository publishes releases. When it does, a note on standard error says so under the table and suggests the `tuff update <id>@<requirement>` that pins the newest one; `--json` carries it as `latest_release` on every git row, including release-pinned ones, where it is the same value as `latest`. This is where Tuff tells you release tags exist: `tuff add` without an `@` never lists them, so an untagged install costs no extra round trip.

For a capability installed at a release, `CURRENT` and `LATEST` are release versions, and `outdated` carries the size of the change the version numbers claim: `major`, `minor`, or `patch`. That is what the author claimed, not what the content shows; `tuff diff <id> --upstream` shows the content. `LATEST` is the newest release the repository has, whether or not the recorded requirement allows it, so an exact pin can read `outdated` while `tuff update` reports it up to date; `tuff update <id>@<requirement>` lifts the pin. In `--json` the status stays `outdated` and the size is a separate `change` key.

The installed tag is also resolved and the commit it names now is compared with the one recorded at install. A tag is mutable by design, so if someone republished different content under the same tag, the row reads `repointed` rather than `up to date`, and a tag the repository no longer has reads `tag missing`. Either finding wins over `outdated`, since it changes what the version you have means, while `LATEST` still shows the newest release. The install itself is unaffected: the lockfile pins the commit, not the tag. This is the same treatment a pack's registry tag gets, and it costs nothing extra, since the one `ls-remote` already lists every tag with its commit.
