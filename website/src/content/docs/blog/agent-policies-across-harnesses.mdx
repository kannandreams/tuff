...
description: How coding harnesses enforce permission rules, and how Tuff compiles one policy into the native rules of Claude Code, Codex, and OpenCode, records what an agent cannot enforce, and detects drift.
date: 2026-09-14
lastUpdated: 2026-09-16
authors: kannan
tags: [policy, security]
excerpt: Each coding harness has its own permission system. A Tuff policy declares deny and ask rules once, compiles them into the native rules of Claude Code, Codex, and OpenCode, records the rules an agent cannot enforce, and fails tuff check when a compiled rule is removed.
---
...
Every command and file below is from tuffcli 0.11.0, which enforces policies in Claude Code, Codex, and OpenCode.
...
<div class="not-content tuff-adapter" role="figure" aria-label="tuff add compiles one policy into Claude Code permission rules and refuses harnesses that cannot enforce it">
  <div class="tuff-adapter-diagram">
    ...
    <div class="tuff-harness-list">
      <div style="--i: 3"><strong>claude</strong><small>.claude/settings.json</small><code>{'deny → Bash(git push '}<em class="tuff-nowrap">{'--force *)'}</em></code><code>{'deny → Read(/**/.env)'}</code><code>{'ask  → Bash(terraform apply *)'}</code><code>{'deny → mcp__github__delete_*'}</code></div>
      <div style="--i: 4"><strong>opencode</strong><small>.opencode/opencode.json</small><code>{'permission.bash → "git push --force *": "deny"'}</code><code>{'permission.read → ".env": "deny"'}</code><code>{'permission → "github_delete_*": "deny"'}</code></div>
      <div style="--i: 5"><strong>codex</strong><small>.codex/rules/tuff.rules</small><code>{'prefix_rule(["git", "push", "--force"], "forbidden")'}</code><code>{'read and mcp rules: recorded, not enforced'}</code></div>
      <div class="is-muted" style="--i: 6"><strong>cursor</strong><small>not enforced yet</small><code>install refused, so no rule is assumed</code></div>
    </div>
  </div>
</div>
...
Codex reads command rules from `.codex/rules/`. Tuff owns one file there and compiles the policy's `command` rules into it. Codex has no project rule for file paths or MCP tools, so two of the four rules have nowhere to go, and `tuff add` refuses the policy rather than installing half of it:
...
error: policy 'infra-guardrails' would not be enforced as written, so it was not installed:
  ...
  Codex: rule 4 (deny mcp "github:delete_*") is not enforced: Tuff does not compile MCP tool rules for Codex yet
...
Codex: rule 4 (deny mcp "github:delete_*") is not enforced, so it was not installed: Tuff does not compile MCP tool rules for Codex yet; recorded in tuff.lock
...
```text title=".codex/rules/tuff.rules"
...
! infra-guardrails         policy codex        rule 4 (deny mcp "github:delete_*") is not enforced: Tuff does not compile MCP tool rules for Codex yet
```

### Compiling to OpenCode
...
`tuff policy matrix` lists each harness, effect, and subject with a coverage of `full`, `partial`, or `unsupported`, the terms the [Hooks Specification](/spec/hooks/) uses, and the native rule each one compiles to:
...
The command then prints a note for every `partial` and `unsupported` row. The notes are where the limits live. Four of them, out of the sixteen it prints:
