...
- Git-sourced capabilities can be installed at a release. `tuff add skill <repo> <name>@1.2.0` installs exactly that release and `<name>@^1.2` the newest one in the range, resolved from the repository's tags without cloning: `v1.4.0` or `1.4.0` for the whole repository, `<name>/v1.4.0` or `<name>-v1.4.0` for one capability in a monorepo, with capability-scoped tags taking precedence when any exist. The lockfile keeps pinning the commit and records the tag and the requirement beside it, with the entry's version now the release's and `version_scheme = "semver"`. A requirement nothing satisfies fails before anything is cloned and lists the releases that exist; a repository with no release tags says how to tag one. Installing without `@` is unchanged and still records the commit SHA.
- `tuff update` respects a release pin. A capability installed at a release moves to the newest release its recorded requirement allows, never to the latest commit, and reports when that release is already installed. `tuff update <id>@<requirement>` records a new requirement and moves, which is how an exact pin is lifted. `--check` names the release and the size of the change the version numbers claim, as in `to 1.4.0 (minor)`.
- `tuff outdated` compares a release-pinned capability against the newest release the repository has, with one `ls-remote` and no clone, and shows the claimed size of the change beside `outdated`; in `--json` it is a separate `change` key. Release tags removed upstream read `tag missing`.
- A git install that no release tag chose now records the version the source declares for itself: `version` in `tuff.toml`, else `version:` or `metadata.version:` in the `SKILL.md` frontmatter, where the Agent Skills specification puts it. The lockfile marks it `version_scheme = "declared"`; a source declaring nothing still records the commit SHA. Because a declared version may not move when the content does, `tuff list` and `tuff outdated` show it as `1.2.0 (declared)` for a git install, `outdated` compares the version declared then with the one declared now and names the claimed size of the change, and a commit that moved without a version bump still reads `outdated`. A local skill without a `tuff.toml` also takes its frontmatter version instead of `0.1.0`. `list --json` and `outdated --json` rows carry `version_scheme`.
- `tuff diff <id> --upstream` on a release-pinned capability compares against the newest release the recorded requirement allows, the same content `tuff update` would install, rather than the latest commit. `tuff diff <id>@<requirement> --upstream` previews a different requirement, so what lifting a pin would change is visible before `tuff update <id>@<requirement>` does it. A note on standard error names the release compared against and the JSON carries it as `upstream`; a capability pinned to a commit still compares against the latest commit.
- A release tag that moved or vanished upstream is reported rather than trusted, as a pack's registry tag already was. `tuff outdated` resolves the installed tag and compares the commit it names now with the one recorded at install: a mismatch reads `repointed` and a deleted tag `tag missing`, both winning over `outdated` while `LATEST` still shows the newest release. `tuff update` on a repointed entry refuses to call it up to date, previews the replacement with `--check`, and replaces the install with `--force`. The lockfile pins the commit, so the install itself was never affected; what changed is what the version claims to be.
- Fixed a git install with a `tuff.toml` recording the commit SHA over the manifest's own version, and `tuff update` on such a source synthesizing a skill manifest instead of reading the `tuff.toml` as `add` does.
...
[Unreleased]: https://github.com/kannandreams/tuff/compare/v0.4.0...HEAD
