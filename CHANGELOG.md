...
- `tuff init` now registers the harnesses a project already contains and gives each one the `tuff-cli-guide` skill in its own layout. A `.claude/` directory or a `CLAUDE.md` file registers `claude`, and a `.cursor/` directory registers `cursor`. Until now the guide was recorded against `open-agents` only, which meant a Claude Code or Cursor session never saw the reference that teaches an agent to drive Tuff, in exactly the projects that had already adopted it. Codex is deliberately not detected: it writes the same `.agents/` root `open-agents` already covers, and its detector matches the directory `init` itself creates.
- A capability that is already installed can now be emitted for a harness it was not installed for. `tuff add .agents/skills/release-checklist --agent claude` records the new target and writes the harness-native output, where it previously refused with `already in the 'open-agents' agent layout`. The recorded source, version, and description are preserved, so adding a harness to a capability installed from Git keeps its repository and resolved revision. A target that is already recorded is still refused, with the same message as before, since re-emitting one is what `tuff update` is for.
