...
- **Policy capabilities can be written and checked, though no agent enforces them yet.** A `type = "policy"` manifest lists rules that narrow what an agent may do: `deny` or `ask`, for a command given as a prefix of its arguments, file paths it must not read or edit, or an MCP tool as `server:tool`. Tuff validates a policy when it is loaded and refuses a rule with no subject or two, an argument containing a space, a path that is absolute or climbs out of the project, or a misspelt field. There is no `allow` effect and a rule using one is refused, because a policy can come from another team's repository or a pack, and one that could grant permissions could quietly widen what an agent may do in every project that installs it. `tuff policy matrix` prints, for every agent, how each kind of rule is enforced, in the same `full`, `partial`, `unsupported` terms the hooks specification uses. No agent compiles policies yet, so every row reads `unsupported`, and `tuff add` of a policy is refused, naming each rule and the agent that would not enforce it: a policy reported as installed where nothing enforces it would be worse than none. Enforcement follows agent by agent, starting with Claude Code's native permission rules.
