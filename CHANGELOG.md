...
- Registry entries publishing both the `streamable-http` and the superseded `sse` transport now install the former rather than whichever was listed first, and an entry offering only `sse` is refused instead of being installed as though it were Streamable HTTP, which wrote a config no harness could use.
...
- MCP servers reached over HTTP can now declare the auth header they need. `[server.headers]` takes the same `{ from_env = "NAME" }` references `[server.env]` already takes, plus an optional `format = "Bearer {}"` for the near-universal case where the header wraps the token, so a manifest still has no field a literal secret can occupy. Each harness gets its own dialect: Claude Code, Codex, and Open Agents expand `${VAR}`, Cursor `${env:VAR}`. `tuff check` catches a header edited by hand, and the post-install reminder names header variables alongside environment ones. Probing these servers with `tuff mcp doctor` is the next step and is not done yet.
