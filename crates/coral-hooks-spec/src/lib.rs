//! Canonical hook metadata for Coral-standard hooks.
//!
//! This crate models the stable vocabulary Coral uses when a hook is authored in
//! Coral's manifest format and then rendered by an adapter into a native harness
//! format. It does not model native harness hook fragments; those remain
//! adapter-owned passthrough data.

use serde::{Deserialize, Serialize};

/// Current version of the Coral hook specification.
pub const SPEC_VERSION: &str = "0.1.0";

/// Canonical Coral hook events.
...
pub struct HookEventSpec {
    ...
    pub event: HookEvent,
    /// Event name used in Coral-standard manifests.
    pub canonical_name: &'static str,
    /// Blocking behavior exposed by the canonical event.
    pub blocking: BlockingScope,
    /// First Coral hook spec version that includes this event.
...
pub struct CompatibilityMatrix {
    /// Coral hook spec version this matrix targets.
...
/// Common payload fields available to most Coral-standard hooks.
