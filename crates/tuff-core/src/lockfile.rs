...
pub fn managed_mcp_entry_status(
    ...
    if crate::mcp::is_toml(&path) {
        return match crate::mcp::toml_entry(&path, capability_id) {
            Ok(Some(current)) => match serde_json::to_vec(&current) {
