...
pub(crate) fn materialize_baseline(
    ...
    let target_entry = entry
        .targets
        .get(target)
        .ok_or_else(|| CoralError::new(format!("no lock entry for '{id}' at target '{target}'")))?;
    ...
    if actual != *expected {
        ...
        {
            return Err(CoralError::new(format!(
                "The recorded baseline for \"{id}\" is not cached, and its local source \"{}\" is no longer available.\n\nRestore the source, reinstall the capability, or run the appropriate Coral command to accept the current content as a new baseline.",
                entry.source_path
            )));
        }
        return Err(CoralError::new(format!(
...
fn materialize_source(
    ...
    let adapter = AdapterKind::from_id(target)
        .ok_or_else(|| CoralError::new(format!("unknown target '{target}'")))?;
    ...
    let manifest = if capability_dir.join("coral.toml").is_file() {
